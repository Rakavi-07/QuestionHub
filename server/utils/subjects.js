 ...
 *
 * This is the single hardcoded source of truth for which subjects exist
 * under which semester. Admins do NOT type in Subject Code / Subject Name
 * — they only ever pick Semester -> Subject from this predefined list when
 * uploading, replacing, or deleting a question paper PDF (future phase).
 *
 * Architecture: One Subject -> One PDF -> that PDF contains ALL previous
 * year question papers for that subject. There is no Year or Month filter,
 * and no separate PDF per year.
 */
...
export const VALID_SEMESTERS = Object.keys(PHYSIOTHERAPY_SUBJECTS).map(Number);

export function isValidSemester(semester) {
  return VALID_SEMESTERS.includes(Number(semester));
}

export function isValidSubjectCode(semester, subjectCode) {
  const subjects = PHYSIOTHERAPY_SUBJECTS[Number(semester)];
